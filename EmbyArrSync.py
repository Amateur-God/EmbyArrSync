...
def get_tvdb_token():
    ...
    data = {
        "apikey": "9a84c23b-fb99-4bd8-be5b-a3513dc94afc",
    }
...
def get_tvdb_id(series_name, series_path):
    ...
    if response.status_code == 200:
        if results := response.json().get('data', []):
            series_slug = slugify(re.sub(r'&', 'and', series_name))
            index = next((i for i, item in enumerate(results) if item['slug'] == series_slug), None)
            if index is not None:
                return results[index].get('tvdb_id')
            print(f"{dt} No exact match found for {series_name}")
        else:
            print(f"{dt} No results found at all for {series_name}")
    else:
        print(f"{dt} Failed to search TVDB for {series_name}: {response.status_code}, {response.text}")
...
def get_movie_info(tmdb_id, movie_name):
    url = f"{RADARR_URL}/movie?tmdbId={tmdb_id}&apikey={RADARR_API_KEY}"
...
def unmonitor_movies(radarr_id, movie_name):
    ...
    if response.status_code == 200:
        movie_info = response.json()
        if movie_info.get('monitored'):
            movie_info['monitored'] = False
            headers = {
                'accept': 'application/json',
                'Content-Type': 'application/json'
            }
            # Use PUT with the FULL OBJECT
            put_response = requests.put(movie_info_url, json=movie_info, headers=headers)
            if put_response.status_code in {200, 202}:
                print(f"Monitoring status updated for {movie_name}.")
            else:
                print(f"Error updating monitoring status for {movie_name}: {put_response.status_code}, {put_response.text}")
        else:
            print(f"{movie_name} is already unmonitored.")
    else:
        print(f"Error fetching movie info for {movie_name}: {response.status_code}, {response.text}")
